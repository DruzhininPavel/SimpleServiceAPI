...
object ImageService {
  ...
  def apply[F[_]](imageRepo: ImagesRepo[F])(implicit F: Applicative[F]): ImageService[F] = new ImageService[F] {
    private def increaseTouchImage(id: Long): Unit =
      OptionT(imageRepo.get(id)).cata((), image => updateImage(id, image.copy(touched = image.touched + 1)))

    override def getAll(): F[List[Image]] = imageRepo.getAll()

    override def getRandomImage(): F[Option[Image]] = {
//      val ans = for {
//        rand <- OptionT(imageRepo.getRandom())
//            _ =  increaseTouchImage(rand.id)
//        image <- OptionT(getImage(rand.id))
//      } yield image
//      ans.value
//
//      def imageProcess(image: Image) = {
//        increaseTouchImage(image.id)
//        image
//      }
//      imageRepo.getRandom().map(_.map(image => imageProcess(image)))
      imageRepo.getRandom().map{
        case Some(image) => imageRepo.unsafeUpdate(image.id, image.copy(touched = image.touched + 1))
      }
    }
